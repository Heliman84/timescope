...
- Hierarchical jobs (#15): Start now walks an opt-in Client → Project binding for an opted-in repo, then a Task-type picker (pinned first, full vocabulary via "Other…", inline conversion of legacy flat jobs into a Task-type via alias); the dashboard groups sessions by "Client › Project › Task-type" (falling back to Task-type alone, or the flat title when unassigned) and now reads its own owned sources instead of the derived `index.jsonl`. All on-disk additions are additive-only (no format-version bumps; `docs/record_format_spec.md` untouched, owed to the storage track).
