...
let start_button: vscode.StatusBarItem;
let pause_button: vscode.StatusBarItem;
let resume_button: vscode.StatusBarItem;
let stop_button: vscode.StatusBarItem;
let timer_item: vscode.StatusBarItem;
let summary_button: vscode.StatusBarItem;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("timescope.start", handle_start),
        vscode.commands.registerCommand("timescope.pause", handle_pause),
        vscode.commands.registerCommand("timescope.resume", handle_resume),
        vscode.commands.registerCommand("timescope.stop", handle_stop),
        vscode.commands.registerCommand("timescope.dashboard",() => handle_dashboard(context)),
        vscode.commands.registerCommand("timescope.rename_job", handle_rename_job)
    );
}
...
function get_workspace_root(): string | null {
    const folders = vscode.workspace.workspaceFolders;
    return folders && folders.length > 0 ? folders[0].uri.fsPath : null;
}

function get_global_data_dir(): string {
...
function get_log_file_path(workspace_root: string | null): string {
    if (workspace_root) {
...
function get_global_jobs_path(): string {
    return path.join(get_global_data_dir(), "jobs.json");
}

function get_workspace_jobs_path(): string | null {
    if (!state.workspace_root) return null;
...
async function ensure_directory_for_file(file_path: string): Promise<void> {
    await fs.promises.mkdir(path.dirname(file_path), { recursive: true });
}

async function load_jobs(file_path: string): Promise<string[]> {
    try {
...
async function save_jobs(file_path: string, jobs: string[]): Promise<void> {
    await fs.promises.mkdir(path.dirname(file_path), { recursive: true });
    const content = JSON.stringify({ jobs }, null, 2);
    await fs.promises.writeFile(file_path, content, "utf8");
}

async function load_all_jobs(): Promise<string[]> {
...
async function rename_job_in_job_lists(old_name: string, new_name: string) {
...
async function rename_job_in_log_file(old_name: string, new_name: string) {
    if (!state.log_file_path) return;
    ...
    const updated = lines.map(line => {
        ...
        if (e.job === old_name) e.job = new_name;
...
async function add_job(job_name: string, save_as_global: boolean): Promise<void> {
    if (save_as_global) {
        const global_path = get_global_jobs_path();
...
async function select_job(): Promise<string | null> {
    const jobs = await load_all_jobs();

    const items: vscode.QuickPickItem[] = jobs.map(j => ({
        label: j
    }));
...
async function append_log_entry(entry: Record<string, unknown>): Promise<void> {
...
async function handle_pause() {
    if (!state.is_running || state.is_paused) return;
...
async function handle_resume() {
    if (!state.is_paused || !state.current_job) return;
...
async function handle_rename_job() {
    ...
    if (!new_name || new_name.trim() === "" || new_name === old_name) return;
...
async function parse_log_file(log_path: string): Promise<any[]> {
    try {
...
async function handle_dashboard(context: vscode.ExtensionContext) {

    const panel = vscode.window.createWebviewPanel(
        "timeTrackerSummary",
        "Time Tracker Summary Dashboard",
