...
function r(e: "start"|"pause"|"resume"|"stop", t: number, job = "job1", task?: string): LogRecord {
    if (e === "stop") return { event: e, job, timestamp: t, task } as any;
...
export function run_event_collection_tests(): void {
    ...
    assert.ok(errs2.some(m => (m.message || '').includes("Unexpected resume")), `expected Unexpected resume, got ${errs2.map(e => e.message).join(';')}`);
    ...
    assert.ok(errs3.some(m => (m.message || '').includes("must begin with start")), `expected must begin message, got ${errs3.map(e => e.message).join(';')}`);
    ...
    assert.ok(errs7.some(e => e.code === "unexpected_start"), `expected unexpected_start, got ${errs7.map(e => e.message).join(';')}`);
    ...
    assert.ok(errs8.some(e => e.code === "unexpected_pause"), `expected unexpected_pause, got ${errs8.map(e => e.message).join(';')}`);

    // Invalid: unknown event type should produce unknown_event
    const seq9 = new EventCollection([{ event: "start", job: "job1", timestamp: 10 } as any, { event: "weird", job: "job1", timestamp: 20 } as any]);
    const errs9 = seq9.validate();
    assert.strictEqual(errs9.length > 0, true, "expected error for unknown event");
    assert.ok(errs9.some(e => e.code === "unknown_event"), `expected unknown_event, got ${errs9.map(e => e.message).join(';')}`);
    ...
    assert.ok(errs10.some(e => e.code === "consecutive_duplicate" || e.code === "unexpected_start"), `expected duplicate or unexpected_start, got ${errs10.map(e => e.message).join(';')}`);
